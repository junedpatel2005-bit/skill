...
const globalForLegalCms = globalThis;
...
function ensureLegalPagesTable(db) {
  db.exec(`
...
function getLegalPageBySlug(slug) {
  const db = getDatabase();
  return db.prepare(
    `
        SELECT slug, title, content, status, updatedAt
...
function getPublishedLegalPageBySlug(slug) {
  const page = getLegalPageBySlug(slug);
  return page?.status === "PUBLISHED" ? page : void 0;
}
function saveLegalPage(slug, input) {
  const db = getDatabase();
