...
function getDatabase$6() {
  if (!globalForUserDb.userDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForUserDb.userDb = new Database(databasePath2);
...
const globalForWebsiteCms = globalThis;
function getDatabase$5() {
  if (!globalForWebsiteCms.websiteCmsDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForWebsiteCms.websiteCmsDb = new Database(databasePath2);
...
function saveWebsitePage(pageKey, input) {
  ...
  ).run(sanitizedContent, input.status, (/* @__PURE__ */ new Date()).toISOString(), pageKey);
...
function getDatabase$4() {
  if (!globalForJobDb.jobDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForJobDb.jobDb = new Database(databasePath2);
...
function getFavoriteJobsByUserId(userId) {
  ...
  return getOpenClientJobs().filter((job) => favoriteIdSet.has(job.id)).sort((a, b) => (favoriteOrder.get(a.id) ?? 0) - (favoriteOrder.get(b.id) ?? 0));
}
...
function getDatabase$3() {
  if (!globalForProjectRequestDb.projectRequestDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForProjectRequestDb.projectRequestDb = new Database(databasePath2);
  }
...
function getDatabase$2() {
  if (!globalForHireDb.hireDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForHireDb.hireDb = new Database(databasePath2);
  }
...
function getDatabase$1() {
  if (!globalForNotificationDb.notificationDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForNotificationDb.notificationDb = new Database(databasePath2);
  }
...
function getUserNotifications(userId, role) {
  ...
  }).filter((notification) => Boolean(notification)).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
...
function getDatabase() {
  if (!globalForAdminDashboardDb.adminDashboardDb) {
    const databasePath2 = path.resolve(process.cwd(), "prisma", "app.db");
    globalForAdminDashboardDb.adminDashboardDb = new Database(databasePath2);
  }
...
async function route(request, url) {
  ...
  if (method === "GET" && pathname === `${API_PREFIX}/reports/tables`) {
    try {
      ...
      if (user.role !== "ADMIN") {
        ...
        visibleTables = allTables.filter(
          (t) => !["ApiToken", "StoredFile", "Faq", "PasswordReset", "VerificationToken", "sqlite_sequence"].includes(t)
        );
      }
  ...
  if (method === "GET" && pathname === `${API_PREFIX}/reports/summary`) {
    try {
      ...
      return json({ role: "ADMIN", cards: [], charts: { monthlyUsers: [], jobsByStatus: [] }, scope: null, generatedAt: (/* @__PURE__ */ new Date()).toISOString(), error: msg }, { status: 500 });
    }
  }
  function buildReportWhereClause(user, table, colNames, filters2) {
    const where = [];
    const params = [];
    if (user.role === "PROFESSIONAL") {
      if (colNames.includes("professionalId")) {
        where.push(`"professionalId" = ?`);
        params.push(user.id);
      } else if (colNames.includes("reviewedBy") && (table === "ProjectReview" || table === "project_reviews")) {
        where.push(`("createdBy" = ? OR "reviewedBy" = ?)`);
        params.push(user.id, user.id);
      }
    } else if (user.role === "CLIENT") {
      if (colNames.includes("clientId")) {
        where.push(`"clientId" = ?`);
        params.push(user.id);
    ...
    if (filters2.id && colNames.includes("id")) {
      where.push(`"id" = ?`);
      params.push(filters2.id);
    }
    if (filters2.po) {
      const poCols = colNames.filter(
        (c) => ["trackingId", "requestId", "orderId", "reference", "poNumber"].includes(c)
      );
      if (poCols.length > 0) {
        const poClause = poCols.map((c) => `"${c}" = ?`).join(" OR ");
        where.push(`(${poClause})`);
        for (let i = 0; i < poCols.length; i++) params.push(filters2.po);
      }
    }
    if (filters2.from && filters2.from.trim() && colNames.includes("createdAt")) {
      where.push(`datetime("createdAt") >= datetime(?)`);
      params.push(filters2.from);
    }
    if (filters2.to && filters2.to.trim() && colNames.includes("createdAt")) {
      where.push(`datetime("createdAt") <= datetime(?)`);
      params.push(filters2.to);
    }
    if (filters2.category && filters2.category.trim() && colNames.includes("category")) {
      where.push(`"category" = ?`);
      params.push(filters2.category);
    }
    if (filters2.userType && filters2.userType.trim() && colNames.includes("role")) {
      where.push(`"role" = ?`);
      params.push(filters2.userType);
    }
    const statusVal = (filters2.status || filters2.jobStatus || filters2.paymentStatus || "").trim();
    if (statusVal && colNames.includes("status")) {
      where.push(`"status" = ?`);
      params.push(statusVal);
    }
    if (filters2.search && filters2.search.trim()) {
      const searchCols = colNames.filter(
        (c) => !["id", "createdAt", "updatedAt", "passwordHash", "googleId"].includes(c)
      );
      if (searchCols.length > 0) {
        const likeClause = searchCols.map((c) => `"${c}" LIKE ?`).join(" OR ");
        where.push(`(${likeClause})`);
        const searchTerm = `%${filters2.search.trim()}%`;
  ...
  if (method === "POST" && pathname === `${API_PREFIX}/reports/preview`) {
    try {
      ...
      const totalRow = db.prepare(`SELECT COUNT(*) as cnt FROM "${table}" ${whereSql}`).get(...params);
      const total = Number(totalRow?.cnt ?? 0);
      const page = Number(payload.page || 1);
      const pageSize = Number(payload.pageSize || 50);
      const offset = (page - 1) * pageSize;
      const rows = db.prepare(`SELECT * FROM "${table}" ${whereSql} ORDER BY "${colNames.includes("id") ? "id" : colNames[0]}" DESC LIMIT ? OFFSET ?`).all(...params, pageSize, offset);
      return json({
        columns: colNames,
        rows,
        total,
  ...
  if (method === "POST" && pathname === `${API_PREFIX}/reports/download`) {
    try {
      ...
      const colNames = cols.map((c) => c.name);
      const { whereSql, params } = buildReportWhereClause(user, table, colNames, payload.filters || {});
      const rows = db.prepare(`SELECT * FROM "${table}" ${whereSql} ORDER BY "${colNames.includes("id") ? "id" : colNames[0]}" DESC LIMIT 100000`).all(...params);
      ...
      if (payload.format === "CSV") {
        ...
        return new Response(csv, { headers: { "content-type": "text/csv", "content-disposition": `attachment; filename="${(payload.reportName || table).replace(/[^a-z0-9.-]/gi, "_")}.csv"` } });
      }
      try {
...
<body>
  ...
  <div class="footer">
    <p>&copy; ${(/* @__PURE__ */ new Date()).getFullYear()} Servio Professional Gateway. All rights reserved. | Confidental Business Report</p>
        ...
        if (!header.startsWith("%PDF-")) throw new ApiError(500, "PDF generation produced invalid output");
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./assets/server-B8npIiGW.js").then((n) => n.s).then(
      (m) => m.default ?? m
    );
  }
...
export {
  getResponse as $,
  getAdminJobRecords as A,
  getAdminManagedUserDetails as B,
  getAdminPaymentTransactions as C,
  getAdminUserStats as D,
  getAdminUsers as E,
  getClientHireRequests as F,
  getClientJobById as G,
  getClientJobsByUserId as H,
  getClientProfileByUserId as I,
  getClientProjectRequests as J,
  getClientTrackedProjects as K,
  getFavoriteJobIds as L,
  getFavoriteJobsByUserId as M,
  getOpenClientJobById as N,
  getOpenClientJobs as O,
  getOrCreateProjectTrackingDetails as P,
  getProfessionalHireNegotiations as Q,
  getProfessionalHireRequests as R,
  getProfessionalProfileByUserId as S,
  getProfessionalProjectRequests as T,
  getProfessionalTrackedProjects as U,
  getProfessionalUsers as V,
  getProfessionalWithdrawals as W,
  getProjectNegotiationsForClient as X,
  getProjectNegotiationsForProfessional as Y,
  getProjectTrackingDetailsByJob as Z,
  getRequest as _,
  cancelProjectTracking as a,
  getUserNotificationPreferences as a0,
  getUserNotifications as a1,
  getUserProjectTransactions as a2,
  hashPassword as a3,
  isFavoriteJob as a4,
  markUserNotificationsRead as a5,
  rateCompletedProject as a6,
  readSessionFromCookieHeader as a7,
  renderErrorPage as a8,
  requestHandler as a9,
  respondToProjectReview as aa,
  saveClientJobSchema as ab,
  setFavoriteJob as ac,
  setResponseHeader as ad,
  startClientHireProject as ae,
  submitProjectCompletion as af,
  updateAdminDisputeStatus as ag,
  updateAdminPayoutStatus as ah,
  updateClientJob as ai,
  updateClientJobStatus as aj,
  updateClientProfileByUserId as ak,
  updateClientProjectRequestStatus as al,
  updateProfessionalAvatarByUserId as am,
  updateProfessionalHireContractStatus as an,
  updateProfessionalProfileByUserId as ao,
  updateProfessionalVerifiedStatusByAdmin as ap,
  updateProfessionalWorkPhotosByUserId as aq,
  updateProjectCompletionStatus as ar,
  updateProjectMilestoneStatus as as,
  updateUserActiveStatusByAdmin as at,
  updateUserNotificationPreferencesByUserId as au,
  updateUserPasswordByAdmin as av,
  updateUserRoleByAdmin as aw,
  userDb_server as ax,
  verifyPassword as ay,
  websitePageCms_server as az,
  clearSessionCookie as b,
  cancelHireProject as c,
  clearUserNotifications as d,
  server as default,
  createClientJob as e,
  createHireContract as f,
  createProfessionalHireNegotiation as g,
  createProfessionalNegotiation as h,
  createProfessionalWithdrawalRequest as i,
  createProjectDispute as j,
  createProjectMilestone as k,
  createProjectRequest as l,
  createProjectRevisionRequest as m,
  createProjectWorkUpload as n,
  createSessionCookie as o,
  deleteClientJob as p,
  deleteProjectMilestone as q,
  deleteProjectRevisionRequest as r,
  deleteProjectWorkUpload as s,
  deleteRejectedHireRequest as t,
  findUserByEmail as u,
  findUserByEmailOrPhone as v,
  findUserById as w,
  getAdminDashboardSnapshot as x,
  getAdminDisputeRecords as y,
  getAdminEarningsReport as z
};
